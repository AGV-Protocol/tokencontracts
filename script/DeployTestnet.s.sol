...
contract DeployScript is Script {
    ...
    function deployOracle() internal {
        ...
        powerToMint = new PowerToMint(
            address(rggp),
            address(oracleVerification),
            admin
        );
    ...
    function configureParameters() internal {
        ...
        // Configure asset-specific parameters in PowerToMint
    ...
    function fundContracts() internal {
        ...
        // Grant minter role to deployer (msg.sender)
        gvt.grantRole(minterRole, deployer);
        
        // Mint directly to vesting vault
        gvt.mint(address(vestingVault), totalVesting);
        
        console.log("  Funded VestingVault with", totalVesting / 10**18, "GVT");
        
        // Revoke temporary minter role from deployer
        gvt.revokeRole(minterRole, deployer);
    }
    ...
    function saveDeploymentArtifacts() internal { //TODO: fix file writing
        ...
        string memory json = string.concat(
            ...
            '  "contracts": {\n',
            '    "GVT": "', vm.toString(address(gvt)), '",\n',
            '    "rGGP": "', vm.toString(address(rggp)), '",\n',
            '    "BondingCurve": "', vm.toString(address(bondingCurve)), '",\n',
            '    "OracleVerification": "', vm.toString(address(oracleVerification)), '",\n',
            '    "PowerToMint": "', vm.toString(address(powerToMint)), '",\n',
            '    "DAOController": "', vm.toString(address(daoController)), '",\n',
            '    "VestingVault": "', vm.toString(address(vestingVault)), '"\n',
            '  }\n',
            '}'
        );

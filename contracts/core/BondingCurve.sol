...
contract BondingCurve is AccessControl, ReentrancyGuard, Pausable {
     ...
     */
    function convert(uint256 rggpAmount, uint256 vestingDays) 
        external 
        nonReentrant 
        whenNotPaused 
    {
        ...
        require(
            vestingDays >= curveParams.minVestingDays && 
            vestingDays <= curveParams.maxVestingDays,
        ...
        require(
            epochConversions[currentEpochNum] + gvtAmount <= epochConfig.maxGVTPerEpoch,
            "Epoch cap exceeded"
        );
        
        // Check treasury capacity
        require(
            totalAllocated + gvtAmount <= treasuryCapacity,
    ...
    function _calculateGVTAmount(uint256 rggpAmount) private view returns (uint256) {
        ...
        uint256 currentRatio = curveParams.baseRatio + 
            (epochConversions[epochConfig.currentEpoch] * curveParams.slope / 10**18);
     ...
     */
    function updateEpochConfig(uint256 duration, uint256 maxGVTPerEpoch) 
        external 
     ...
     */
    function updateTreasuryCapacity(uint256 newCapacity) 
        external 
        onlyRole(DEFAULT_ADMIN_ROLE) 
    {
     ...
     */
    function previewConversion(uint256 rggpAmount, uint256 vestingDays) 
        external 
        view 
        returns (uint256) 
    {
        ...
        if (vestingDays >= curveParams.minVestingDays && 
            vestingDays <= curveParams.maxVestingDays) {
            uint256 discountBonus = ((vestingDays - curveParams.minVestingDays) * curveParams.discount) / 
                                    (curveParams.maxVestingDays - curveParams.minVestingDays);
            gvtAmount = gvtAmount + (gvtAmount * discountBonus / 10000);
     ...
     */
    function withdrawRGGP(address to, uint256 amount) 
        external 

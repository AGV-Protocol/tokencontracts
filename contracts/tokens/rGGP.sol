...
contract rGGP is ERC20, ERC20Burnable, AccessControl, Pausable {
    ...
    enum AssetType { SOLAR, ORCHARD, COMPUTE }
     ...
     */
    function setMintRate(AssetType assetType, uint256 rate) 
        external 
     ...
     */
    function setEpochCap(AssetType assetType, uint256 cap) 
        external 
        onlyRole(DEFAULT_ADMIN_ROLE) 
    {
    ...
    function mintFromOutput(
        ...
        require(
            epochMinted[currentEpochNum][assetType] + mintAmount <= epochCap[assetType],
            "Epoch cap exceeded"
        );
     ...
     */
    function revokeMint(bytes32 mintId, string calldata reason) 
        external 
